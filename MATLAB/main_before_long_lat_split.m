...
%% Lateral Motion Solutions

%%%
% Finding Phugoid & Short Eigenvalues
%%%

if abs(real(Lat_lambda(3))) > abs(real(Lat_lambda(1)))
    
    Lat_long_eta    = real(Lat_lambda(1));
    Lat_short_eta   = real(Lat_lambda(3));
    
    Lat_long_omega  = imag(Lat_lambda(1));
    Lat_short_omega = imag(Lat_lambda(3));

else

    Lat_long_eta    = real(Lat_lambda(3));
    Lat_short_eta   = real(Lat_lambda(1));
    
    Lat_long_omega  = imag(Lat_lambda(3));
    Lat_short_omega = imag(Lat_lambda(1));
...
Lat_long_t_half     = 0.69 / abs(Lat_long_eta);
Lat_long_period     = 2 * pi / Lat_long_omega;
Lat_long_N_half     = 0.110 * abs(Lat_long_omega) / abs(Lat_long_eta);

%%%
% Phugoid (Lat) Period Solution Approximations
%%%

Lat_omega_n_p          = sqrt(-Z_u * g / u_0);
Lat_zeta_p             = - X_u / (2 * Lat_omega_n_p);
Lat_eigen_1_2_p_real   = - Lat_zeta_p * Lat_omega_n_p;
Lat_eigen_1_2_p_imag   = Lat_omega_n_p * sqrt(1 - Lat_zeta_p ^ 2);
Lat_period_p           = 2 * pi / Lat_eigen_1_2_p_imag;
Lat_t_half_p           = 0.69 / abs(Lat_eigen_1_2_p_real);
Lat_N_half_p           = 0.110 * Lat_omega_n_p / abs(Lat_eigen_1_2_p_real);

%%%
% Short Period Solution
%%%

Lat_short_t_half       = 0.69 / abs(Lat_short_eta);
Lat_short_period       = 2 * pi / Lat_short_omega;
Lat_short_N_half       = 0.110 * abs(Lat_short_omega) / abs(Lat_short_eta);

%%%
% Short Period Solution Approximations
%%%

Lat_omega_n_sp         = sqrt(Z_alpha * M_q / u_0 - M_alpha);
Lat_zeta_sp            = - (M_q + M_alpha_dot + Z_alpha / u_0) / (2 * Lat_omega_n_sp);
Lat_eigen_1_2_sp_real  = - Lat_zeta_sp * Lat_omega_n_sp;
Lat_eigen_1_2_sp_imag  = Lat_omega_n_sp * sqrt(1 - Lat_zeta_sp ^ 2);
Lat_period_sp          = 2 * pi / Lat_eigen_1_2_sp_imag;
Lat_t_half_sp          = 0.69 / abs(Lat_eigen_1_2_sp_real);
Lat_N_half_sp          = 0.110 * Lat_omega_n_sp / abs(Lat_eigen_1_2_sp_real);
...
table_Long_ApproxComp_col_Types = [""+...
...
table_Long_ApproxComp_col_ExactMethod = [""+...
...
table_Long_ApproxComp_col_Approximation = [""+...
...
table_Long_ApproxComp_col_Difference = [""+...
...
table_Long_ApproxComp = table(table_Long_ApproxComp_col_ExactMethod, table_Long_ApproxComp_col_Approximation, table_Long_ApproxComp_col_Difference);
table_Long_ApproxComp.Properties.VariableNames = ["Exact Method", "Approximate Method", "Difference"];
table_Long_ApproxComp.Properties.RowNames = table_Long_ApproxComp_col_Types;

%%%
% Longitudinal Eigenvalue & Eigenvector Table
%%%

table_Long_eigen_col = 0;
...
table_Lat_ApproxComp_col_Types = [""+...
    "Phugoid Half Amp Time (t_(1/2))";
    "Phugoid Period (P)";
    "Phugoid Frequency (omega)";
    "Short Half Amp Time (t_(1/2))";
...
table_Lat_ApproxComp_col_ExactMethod = [""+...
    num2str(Lat_long_t_half) + " s";
    num2str(Lat_long_period) + " s";
    num2str(Lat_long_omega) + " rad/s"
    num2str(Lat_short_t_half) + " s";
    num2str(Lat_short_period) + " s";
    num2str(Lat_short_omega) + " rad/s"];
           
table_Lat_ApproxComp_col_Approximation = [""+...
    num2str(Lat_t_half_p) + " s";
    num2str(Lat_period_p) + " s";
    num2str(Lat_omega_n_p) + " rad/s"
    num2str(Lat_t_half_sp) + " s";
    num2str(Lat_period_sp) + " s";
    num2str(Lat_omega_n_sp) + " rad/s"];
    
table_Lat_ApproxComp_col_Difference = [""+...
    num2str(100 * abs((Lat_long_t_half - Lat_t_half_p) / Lat_long_t_half)) + "%";
    num2str(100 * abs((Lat_long_period - Lat_period_p) / Lat_long_period)) + "%";
    num2str(100 * abs((Lat_long_omega - Lat_omega_n_p) / Lat_long_omega)) + "%";
    num2str(100 * abs((Lat_short_t_half - Lat_t_half_sp) / Lat_short_t_half)) + "%";
    num2str(100 * abs((Lat_short_period - Lat_period_sp) / Lat_short_period)) + "%";
    num2str(100 * abs((Lat_short_omega - Lat_omega_n_sp) / Lat_short_omega)) + "%"];

table_Lat_ApproxComp = table(table_Lat_ApproxComp_col_ExactMethod, table_Lat_ApproxComp_col_Approximation, table_Lat_ApproxComp_col_Difference);
table_Lat_ApproxComp.Properties.VariableNames = ["Exact Method", "Approximate Method", "Difference"];
table_Lat_ApproxComp.Properties.RowNames = table_Lat_ApproxComp_col_Types;
